...
{
    /// <summary>
    /// 减少护甲耐久消耗概率（强制扣除时无效）补丁
    /// 配置项: CheckReduceArmorDurability
    /// 功能: 护甲耐久消耗判定时，根据气运减少消耗的概率
    /// </summary>
    public static class CheckReduceArmorDurabilityPatch
    {
        /// <summary>
        /// CheckReduceArmorDurability 功能专用的替换方法信息
        /// </summary>
        private static class Replacements
        {
            /// <summary>
            /// CheckReduceArmorDurability 专用的 CheckPercentProb False 替换
            /// </summary>
            public static readonly ReplacementMethodInfo CheckPercentProbFalse = new ReplacementMethodInfo
            {
                Type = typeof(CheckReduceArmorDurabilityPatch),
                MethodName = nameof(CheckPercentProbFalse_Method)
            };
        }

        /// <summary>
        /// CheckReduceArmorDurability 功能专用的 CheckPercentProb 替换方法
        /// 支持 CheckReduceArmorDurability 功能的独立气运设置，倾向失败（减少耐久消耗）
        /// </summary>
        public static bool CheckPercentProbFalse_Method(this IRandomSource randomSource, int percent)
        {
            return LuckyCalculator.Calc_Random_CheckPercentProb_False_By_Luck(randomSource, percent, "CheckReduceArmorDurability");
        }

        /// <summary>
        /// 应用减少护甲耐久消耗概率（强制扣除时无效）补丁
        /// </summary>
        /// <param name="harmony">Harmony 实例</param>
        /// <returns>补丁应用是否成功</returns>
        public static bool Apply(Harmony harmony)
        {
            if (!ConfigManager.IsFeatureEnabled("CheckReduceArmorDurability")) return false;

            var OriginalMethod = new OriginalMethodInfo
            {
                Type = typeof(GameData.Domains.Combat.CombatContext),
                MethodName = "CheckReduceArmorDurability",
                Parameters = new Type[] { typeof(sbyte) }
            };

            var patchBuilder = GenericTranspiler.CreatePatchBuilder(
                    "CheckReduceArmorDurability",
                    OriginalMethod);
