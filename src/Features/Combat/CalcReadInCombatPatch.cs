...
namespace QuantumMaster.Features.Combat
{
    ...
    /// </summary>
    public static class CalcReadInCombatPatch
    {
        /// <summary>
        /// 战斗中读书计算功能专用的替换方法信息
        /// </summary>
        private static class Replacements
        {
            ...
            {
                Type = typeof(CalcReadInCombatPatch),
                MethodName = nameof(CheckPercentProbTrue_Method)
            };
        }

        /// <summary>
        /// 战斗中读书计算功能专用的 CheckPercentProb 替换方法（期望成功）
        /// </summary>
        public static bool CheckPercentProbTrue_Method(this IRandomSource randomSource, int percent)
        {
            return LuckyCalculator.Calc_Random_CheckPercentProb_True_By_Luck(randomSource, percent, "CalcReadInCombat");
        }
        ...
        {
            if (!ConfigManager.IsFeatureEnabled("CalcReadInCombat")) return false;
