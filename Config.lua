return {
    Title = "[测试]天机秘术-气运之子(Reborn)",
    Author = "dai",
    Cover = "cover2.jpg",
    BackendPlugins = {
        "QuantumMaster.dll",
    },
    DefaultSettings = {
        {
            SettingType = "Dropdown",
            Key = "LuckyLevel",
            DisplayName = "气运",
            Description = [[选择自己气运，不同的气运有不同的效果。
目前功能还在编写中，只有在描述中注明【气运】的功能，才会受到气运的影响
命途多舛: 做任何事情都极度容易失败，获取的随机收益都会巨幅减少
时运不济：做任何事情都非常容易失败，获取的随机收益都会中幅减少
顺风顺水：关闭本MOD
左右逢源：做任何事情都相对容易成功，获取的随机收益都会小幅增加
心想事成：做任何事情都非常容易成功，获取的随机收益都会中幅增加
福星高照：做任何事情都极度容易成功，获取的随机收益都会大幅增加
洪福齐天：做任何事情都几乎一定成功，获取的随机收益都会巨幅增加
气运之子：做任何事情都必然一定成功，获取的随机收益都是理论上限（概率为0的情况下无效）]],
            Options = {
                "命途多舛",
                "时运不济",
                "顺风顺水",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 7,
        },
        {
            SettingType = "Dropdown",
            Key = "steal",
            DisplayName = "【气运】偷窃",
            Description = "太吾发起偷窃时成功概率根据气运增加，他人对太吾偷窃时成功概率根据气运减少",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "scam",
            DisplayName = "【气运】唬骗",
            Description = "太吾发起唬骗时成功概率根据气运增加，他人对太吾唬骗时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Dropdown",
            Key = "rob",
            DisplayName = "【气运】抢劫",
            Description = "太吾发起抢劫时成功概率根据气运增加，他人对太吾抢劫时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Dropdown",
            Key = "stealLifeSkill",
            DisplayName = "【气运】偷学生活技能",
            Description = "太吾偷学生活技能时成功概率根据气运增加，他人对太吾偷学时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Dropdown",
            Key = "stealCombatSkill",
            DisplayName = "【气运】偷学战斗技能",
            Description = "太吾偷学战斗技能时成功概率根据气运增加，他人对太吾偷学时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Dropdown",
            Key = "poison",
            DisplayName = "【气运】下毒",
            Description = "太吾发起下毒时成功概率根据气运增加，他人对太吾下毒时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Dropdown",
            Key = "plotHarm",
            DisplayName = "【气运】暗害",
            Description = "太吾发起暗害时成功概率根据气运增加，他人对太吾暗害时成功概率根据气运减少",
            DefaultValue = 0,
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
        },
        {
            SettingType = "Slider",
            Key = "genderControl",
            DisplayName = "生成性别控制",
            Description = "生成角色时，如果是随机的性别，则按照选择进行修改。对于指定性别的NPC生成无效。这个选项控制的是男性的概率，女性的概率就是100%减去男性的概率。（此处控制的是真实性别）",
            MinValue = 0,
            MaxValue = 100,
            StepSize = 1,
            DefaultValue = 50,
        },
        {
            SettingType = "Dropdown",
            Key = "sexualOrientationControl",
            DisplayName = "性取向控制",
            Description = "太吾世界中每个人在诞生的时候都有20%概率是同性恋，可以通过此设置控制所有人（包括开启这个功能之前出生的人）的性取向。性取向会影响表白结婚之类的概率。",
            Options = {
                "关闭功能",
                "全体同性恋",
                "禁止同性恋"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CreateBuildingArea",
            DisplayName = "【气运】世界初始建筑",
            Description = "生成世界时，根据气运增加产业中的建筑和资源点的初始等级，以及生成数量",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ropeOrSword",
            DisplayName = "【气运】绳子绑人",
            Description = "事件触发时，绳子绑架或者剑柄救人的成功率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "collectResource",
            DisplayName = "【气运】采集资源引子",
            Description = "采集资源获取引子的概率和品级根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "GetCollectResourceAmount",
            DisplayName = "【气运】采集资源数量",
            Description = "采集数量根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OfflineUpdateShopManagement",
            DisplayName = "【气运】太吾村经营成功率",
            Description = "出售物品成功率、采集物品成功率和品级根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "BuildingRandomCorrection",
            DisplayName = "【气运】太吾村经营收益",
            Description = "产业收益（木材、金铁、金币、威望等）的数量根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "UpdateShopBuildingTeach",
            DisplayName = "【气运】村民经营资质增加概率",
            Description = "村名在经营时，资质增加的概率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "BuildingManageHarvestSpecialSuccessRate",
            DisplayName = "【气运】赌坊与青楼基础暴击率",
            Description = "赌坊与青楼的经营暴击率根据气运增加（暴击为3倍收益）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ApplyLifeSkillCombatResult",
            DisplayName = "【气运】教艺读书&周天",
            Description = "如果较艺读书&周天的概率大于0，那么触发几率根据气运增加（如果评价过低概率可能为0）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CalcReadInCombat",
            DisplayName = "【气运】战斗读书",
            Description = "如果战斗读书的概率大于0，那么触发几率根据气运增加（如果评价过低概率可能为0）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CalcQiQrtInCombat",
            DisplayName = "【气运】战斗周天运转",
            Description = "如果战斗周天运转的概率大于0，那么触发几率根据气运增加（如果评价过低概率可能为0）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CalcLootItem",
            DisplayName = "【气运】战利品掉落概率",
            Description = "战利品掉落概率不为0，那么掉落的概率根据气运增加（原本逻辑是对每个战利品进行判断是否掉落）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CheckReduceWeaponDurability",
            DisplayName = "【气运】减少武器耐久消耗概率（强制扣除时无效）",
            Description = "武器耐久消耗判定时，根据气运减少消耗的概率",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 3,
        },
        {
            SettingType = "Dropdown",
            Key = "CheckReduceArmorDurability",
            DisplayName = "【气运】减少护甲耐久消耗概率（强制扣除时无效）",
            Description = "护甲耐久消耗判定时，根据气运减少消耗的概率",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 3,
        },
        {
            SettingType = "Dropdown",
            Key = "InitPathContent",
            DisplayName = "【气运】奇遇收获",
            Description = "奇遇收获资源时，数量根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "GetStrategyProgressAddValue",
            DisplayName = "【气运】读书进度策略",
            Description = "读书策略进度增加值根据设置的气运在浮动区间内增加或者减少",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "SetReadingStrategy",
            DisplayName = "【气运】读书效率策略",
            Description = "技艺书籍的效率增加策略进度增加值根据设置的气运在浮动区间内增加或者减少",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ParallelUpdateOnMonthChange",
            DisplayName = "【气运】过月地块资源恢复",
            Description = "地块每月资源恢复数量根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OfflineCalcGeneralAction_TeachSkill",
            DisplayName = "【气运】NPC（包括太吾）受到指点的成功率",
            Description = "如果太吾受到NPC指点，指点的成功率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Toggle",
            Key = "InitResources",
            DisplayName = "初始生成地格资源最大",
            Description = "生成世界时，每个地块上的资源为浮动区间的最大值以及地块上当前可采集的资源，受到难度的影响",
            DefaultValue = true,
        },
        {
            SettingType = "Dropdown",
            Key = "CatchCricket",
            DisplayName = "【气运】抓蛐蛐基础成功率",
            Description = "抓蛐蛐时，基础成功率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CatchCricketDouble",
            DisplayName = "【气运】抓到双蛐蛐概率",
            Description = "抓蛐蛐时，抓到2只蛐蛐的概率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "CheckCricketIsSmart",
            DisplayName = "【气运】蛐蛐神采非凡概率",
            Description = "蛐蛐进行神采非凡判定时，成功率根据气运增加。原版为20%的概率。但是需要注意，必须部位符合的蛐蛐才有可能神采非凡，这是原版的逻辑。",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "GetCurrReadingEventBonusRate",
            DisplayName = "【气运】灵光一闪",
            Description = "灵光一闪触发概率根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Toggle",
            Key = "GeneratePageIncompleteState",
            DisplayName = "生成书籍参数",
            Description = "生成书籍时，完整的书页为浮动区间的最大值，亡佚书页为浮动区间最小值",
            DefaultValue = true,
        },
        {
            SettingType = "Toggle",
            Key = "FixedPagePos",
            DisplayName = "完整书页固定靠前",
            Description = "需要开启生成书籍参数的功能，开启这个功能完整的书页会出现在书本的前篇位置",
            DefaultValue = false,
        },
        {
            SettingType = "Toggle",
            Key = "CricketInitialize",
            DisplayName = "健康的蛐蛐",
            Description = "生成蛐蛐时，必定生成耐久上限为理论上限值的，不受伤的蛐蛐",
            DefaultValue = true,
        },
        {
            SettingType = "Dropdown",
            Key = "CalcNeigongLoopingEffect",
            DisplayName = "【气运】周天内力获取",
            Description = "周天运转时，获得的内力根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "GetQiArtStrategyDeltaNeiliBonus",
            DisplayName = "【气运】周天内力策略收益最大",
            Description = "周天运转时，通过策略获得的内力根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "TryAddLoopingEvent",
            DisplayName = "【气运】天人感应",
            Description = "尝试触发天人感应时，成功率根据设置的气运增加或者减少",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ChoosyGetMaterial",
            DisplayName = "【气运】精挑细选品质升级",
            Description = "精挑细选时，品质升级机制只要概率不是0，则根据气运增加（不是每次都获取最高品）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "AddChoosyRemainUpgradeData",
            DisplayName = "【气运】精挑细选过月累计最大",
            Description = "精挑细选时，每个月都会增加精挑细选的升级概率和数量，这个增加的数值是随机的，开启后可以让增加的数值根据气运增加",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect1",
            DisplayName = "读书策略，第一个策略指定",
            Description = "第一个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 1,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect2",
            DisplayName = "读书策略，第二个策略指定",
            Description = "第二个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 1,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect3",
            DisplayName = "读书策略，第三个策略指定",
            Description = "第三个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 1,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect4",
            DisplayName = "读书策略，第四个策略指定",
            Description = "第四个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 3,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect5",
            DisplayName = "读书策略，第五个策略指定",
            Description = "第五个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 3,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect6",
            DisplayName = "读书策略，第六个策略指定",
            Description = "第六个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 3,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect7",
            DisplayName = "读书策略，第七个策略指定",
            Description = "第七个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 5,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect8",
            DisplayName = "读书策略，第八个策略指定",
            Description = "第八个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 5,
        },
        {
            SettingType = "Dropdown",
            Key = "BookStrategiesSelect9",
            DisplayName = "读书策略，第九个策略指定",
            Description = "第九个策略指定",
            Options = {
                "使用原版",
                "口诵心惟",
                "行思坐忆",
                "独见独知",
                "奇思妙想",
                "照猫画虎",
                "按图索骥",
                "寻章摘句",
                "含英咀华",
                "温故知新",
                "融会贯通",
                "枕经席文",
                "十行俱下",
                "春诵夏弦",
                "推敲化演",
                "心领得间",
                "迁思回虑",
                "囫囵吞枣",
                "大智若愚",
                "义父注解",
            },
            DefaultValue = 5,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect1",
            DisplayName = "周天策略，第一个策略指定",
            Description = "第一个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect2",
            DisplayName = "周天策略，第二个策略指定",
            Description = "第二个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect3",
            DisplayName = "周天策略，第三个策略指定",
            Description = "第三个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect4",
            DisplayName = "周天策略，第四个策略指定",
            Description = "第四个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect5",
            DisplayName = "周天策略，第五个策略指定",
            Description = "第五个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "QiArtStrategiesSelect6",
            DisplayName = "周天策略，第六个策略指定",
            Description = "第六个周天策略指定",
            Options = {
                "使用原版",
                "气沉丹田",
                "五气调营",
                "通经达脉",
                "洞开气海",
                "调形启气",
                "贯气于顶",
                "八脉充盈",
                "洗筋伐髓",
                "真力澎湃",
                "川流不息",
                "窍通络活",
                "炼气调神",
                "内息绵绵",
                "修息生长",
                "运转奇经",
                "云蒸泉涌",
                "天人感应",
                "意明神秀",
                "金刚气体",
                "紫霞气体",
                "玄阴气体",
                "纯阳气体",
                "归元气体",
                "混元气体",
                "化土生金",
                "生水还金",
                "克木成金",
                "化水生木",
                "生火还木",
                "克土成木",
                "化金生水",
                "生木还水",
                "克火成水",
                "化木生火",
                "生土还火",
                "克金成火",
                "化火生土",
                "生金还土",
                "克水还土",
                "祛虚还实",
                "返本归元",
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Toggle",
            Key = "SetSectMemberApproveTaiwu",
            DisplayName = "门派初始支持度",
            Description = "太吾第一次向门派发出拜帖的时候，会默认获取部分人的支持度，具体的人员是随机的，开启后会获取尽可能高品人员的支持度。理论上这获取的支持度会更高",
            DefaultValue = true,
        },
        {
            SettingType = "Dropdown",
            Key = "UpdateCaravansMove",
            DisplayName = "【气运】商队抢劫概率",
            Description = "商队移动时，被抢概率降低",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OnCaravanArrive",
            DisplayName = "【气运】商队收益暴击率",
            Description = "商队获得暴击收益的概率提升",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OnRobGraveOptionResource",
            DisplayName = "【气运】盗墓获得资源概率增加",
            Description = "增加盗墓获得资源的概率（资源和物品只能获得一种，建议只开物品）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OnRobGraveOptionItem",
            DisplayName = "【气运】盗墓获得物品概率增加",
            Description = "增加盗墓获得物品的概率（资源和物品只能获得一种，建议只开物品）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OnRobGraveOptionNothingHappen",
            DisplayName = "【气运】盗墓无事发生概率降低",
            Description = "降低盗墓无事发生的概率",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "OnRobGraveOptionMeetSkeleton",
            DisplayName = "【气运】盗墓遇到骷髅概率降低，【注意】此选项会导致血犼剧情需要盗墓多次才能触发",
            Description = "降低盗墓遇到骷髅的概率，血犼的剧情需要碰到骷髅才能触发，开启此选项会导致需要盗墓多次才能触发血犼剧情（因为过血犼剧情的时候，每次没有遇到骷髅，下次遇到骷髅的概率会叠加。只要概率大于等于100%，天机秘术就会失效，这个时候就能碰到骷髅了，但我还是建议你做血犼剧情的时候关掉这个选项）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "DebateGameTryForceWin",
            DisplayName = "【气运】较艺迫使认输成功率",
            Description = "概率上升",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "UpdateResourceBlockBuildingCoreProducing",
            DisplayName = "【气运】村庄资源点获得心材概率",
            Description = "过月时，太吾村内资源采集建筑采集到资源点心材的概率（有内置冷却时间，低品3个月，高品6个月）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ParallelCreateNewbornChildren",
            DisplayName = "【气运】多胞胎概率",
            Description = "孩子是多胞胎的概率",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "ParallelUpdatePregnantState",
            DisplayName = "【气运】胎教触发概率提升",
            Description = "过月时触发胎教的概率提升，原本概率为75%",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "GenerateMaxDurability",
            DisplayName = "【气运】物品耐久上限最大",
            Description = "生成物品时，物品的初始耐久为理论最大值（原逻辑是 上限值的一半-上限值随机）",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        },
        {
            SettingType = "Dropdown",
            Key = "FindTreasureStandard",
            DisplayName = "【气运】挖宝概率提升",
            Description = "挖宝时，挖到宝物的概率提升",
            Options = {
                "跟随全局",
                "命途多舛",
                "时运不济",
                "顺风顺水(关闭功能)",
                "左右逢源",
                "心想事成",
                "福星高照",
                "洪福齐天",
                "气运之子"
            },
            DefaultValue = 0,
        }
    },
}
