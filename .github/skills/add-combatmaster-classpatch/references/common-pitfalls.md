...
```csharp
[HarmonyPostfix]
public static void Postfix(SomeCombatSkill __instance, ref int __result)
{
...
❌ CombatMaster 中使用 QM 的配置管理器：
```csharp
ConfigManager.IsFeatureEnabled("...")  // 错误！
```

✅ CombatMaster 使用 CombatConfigManager：
```csharp
CombatConfigManager.IsFeatureEnabled("...")  // 正确
```
