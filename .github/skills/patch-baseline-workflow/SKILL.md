...
description: '在 QuantumMaster / CombatMaster 维护 patch 基线、版本对比与更新流程。适用于：按 ConfigManager/CombatConfigManager 配置项生成 patch registry、保存目标函数完整代码快照、保留版本化游戏源码、比较两个版本基线、识别 InternalConsumption 内部消费配置项、生成更新审查模板和中文校对表、输出三级结论（可跳过/人工查看/需要更新）、在补丁适配后回写新基线。触发词：patch baseline、补丁基线、版本对比、更新流程、函数快照、游戏源码快照、registry、compare、update review、internal consumption、三级结论、校对表。'
---
...
6. 输出中文对比表格，并标注三级结论、建议和备注。
7. 在 patch 适配完成后回写新版本基线。
...
5. 在 comparison 目录额外生成 `review-triage.md` 中文校对表，至少包含以下列：
  1. 模块
  2. 配置项/功能
  3. Patch 类
  4. 目标函数
...
6. 三级结论默认使用以下固定值：
  ...
  2. `2-人工查看`
  3. `3-需要更新`
7. 表格行必须按三级结论排序输出：`3-需要更新` 在最前，`2-人工查看` 第二，`1-可跳过` 最后。
8. 生成中文校对表时，必须把“结论为什么成立”写进“建议”或“备注”，不能只填状态。

### C. 更新处置

1. 优先查看 `comparison.md` 摘要。
2. 再查看 `review-triage.md`，先按三级结论分流：
  1. `3-需要更新`
  2. `2-人工查看`
  3. `1-可跳过`
3. 然后按 `update-review-template.json` 中的推荐动作分流：
   1. `patch-redesign-needed`
   2. `patch-change-needed`
   3. `doc-only`
   4. `no-update-needed`
4. 如果需要改 patch，完成代码修改后，重新对目标版本跑一次 `Collect`，刷新基线。

### C1. 三级结论判定规则

生成 `review-triage.md` 时，优先按下面规则判断：

1. `1-可跳过`
  1. 能确认现有 patch 不依赖本次变更的代码位置。
  2. 对于 `ClassPatch`，签名和返回值/参数语义未变，补丁仍直接作用于相同接口。
  3. 对于 `PatchBuilder` / `Hybrid`，旧版和新版的目标调用仍在原函数内，签名兼容，replacement 对应的 occurrence 没有漂移。
2. `2-人工查看`
  1. 任一侧目标函数快照缺失，无法做可靠函数级比对。
  2. comparison 只显示 `method-code-changed`，但现有证据不足以证明 patch 安全或失效。
  3. 变更触及 patch 附近代码，但不能确定是否影响 replacement 语义。
3. `3-需要更新`
  1. 目标函数签名变化，且现有 patch 依赖精确参数匹配。
  2. replacement 依赖的 occurrence 明确变化，或对应调用已删除、挪位、改成其他调用。
  3. 返回值/参数语义已变化，导致现有 `ClassPatch` / `PatchBuilder` 结论不再成立。
4. 对 `PatchBuilder` / `Hybrid` 条目，除 comparison 原始结果外，还应额外对比新旧函数首行声明；即使参数个数不变，只要参数类型变化，也应优先考虑 `3-需要更新`。
...
2. 新版本函数代码快照
3. 新版本 registry
4. `review-triage.md` 中的三级结论、建议和备注
5. 更新审查模板中的最终结论
...
6. 如果任一侧目标函数快照缺失，不要把条目武断标成 `1-可跳过` 或 `3-需要更新`，默认进入 `2-人工查看`，并在备注里写明缺失原因。
7. 如果 `PatchBuilder` / `Hybrid` 的目标函数首行签名已变化，尤其是参数类型变化，不要只因为 `comparison.md` 仍是 `method-code-changed` 就判为可跳过，应优先标记为 `3-需要更新`。
...
6. `review-triage.md` 是否包含中文表格、三级结论、建议和备注。
7. `update-review-template.json` 是否适合人工继续填写最终更新决定。
